...
class ExecuteTask(SequentialTaskSet):
    ...
    locust -f stress_test/kairon_stress_test.py --headless -u 1000 -r 100 --host=http://23.251.147.214:8080
    u: number of users
    ...
    To run from UI:
    locust -f stress_test/kairon_stress_test.py -u 1000 -r 100 --host=http://23.251.147.214:8080
    """
        ...
        def set_endpoint(self):
            with self.client.put(f"/api/bot/{self.user.bot_id}/endpoint",
                                 json={"bot_endpoint": {"url": "http://23.251.147.214:5005/"},
                                       ...
                                       "tracker_endpoint": {"url": "mongodb://23.251.147.214:27017", "db": "rasa"}},
                                 headers={"Authorization": self.user.auth_token},
                                 catch_response=True) as response:
                if response.text is None or not response.text.strip():
                    logging.error(inspect.stack()[0][3] + " Failed: response is None")
