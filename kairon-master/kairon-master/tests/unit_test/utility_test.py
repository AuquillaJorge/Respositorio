...
class TestUtility:
    ...
    def test_get_action_url(self, monkeypatch):
        ...
        assert actual.url == "http://kairon.23.251.147.214:5055/webhook"
    ...
    def test_initiate_apm_client_with_url_present(self, monkeypatch):
        ...
        assert client == {"SERVER_URL": "http://23.251.147.214:8082",
                          "SERVICE_NAME": "kairon",
    ...
    def test_extract_db_config_without_login(self):
        config = Utility.extract_db_config("mongodb://23.251.147.214/test")
        assert config['db'] == "test"
        assert config['username'] is None
        assert config['password'] is None
        assert config['host'] == "mongodb://23.251.147.214"
        assert len(config["options"]) == 0

    def test_extract_db_config_with_login(self):
        config = Utility.extract_db_config("mongodb://admin:admin@23.251.147.214/test?authSource=admin")
        assert config['db'] == "test"
    ...
    def test_get_event_server_url(self):
        assert Utility.get_event_server_url() == 'http://23.251.147.214:5001'
    ...
    def test_validate_smtp_valid(self):
        with patch('kairon.shared.utils.SMTP', autospec=True) as mock:
            assert Utility.validate_smtp("23.251.147.214", 25)
    ...
    async def test_websocket_request(self):
        url = 'ws://23.251.147.214/events/bot_id'
        msg = 'hello'
    ...
    def test_save_and_publish_auditlog_action_save(self, monkeypatch):
        ...
        event_config = EventConfig(bot=bot,
                                   user=user,
                                   ws_url="http://23.251.147.214:5000/event_url")
        kwargs = {"action": "save"}
