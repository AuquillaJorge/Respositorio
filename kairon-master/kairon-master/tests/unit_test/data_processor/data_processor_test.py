...
class TestMongoProcessor:
    ...
    def test_start_training_done_with_intrumentation(self, monkeypatch):
        ...
        monkeypatch.setitem(Utility.environment["elasticsearch"], 'apm_server_url', "http://23.251.147.214:8082")
    ...
    def test_add_endpoints_add_bot_endpoint(self):
        processor = MongoProcessor()
        config = {"bot_endpoint": {"url": "http://23.251.147.214:5000/"}}
        processor.add_endpoints(config, bot="tests1", user="testUser")
        endpoint = processor.get_endpoints("tests1")
        assert endpoint["bot_endpoint"].get("url") == "http://23.251.147.214:5000/"
        assert endpoint.get("action_endpoint") is None
        assert endpoint.get("tracker") is None
    ...
    def test_add_endpoints_add_action_endpoint(self):
        processor = MongoProcessor()
        config = {"action_endpoint": {"url": "http://23.251.147.214:5000/"}}
        processor.add_endpoints(config, bot="tests2", user="testUser")
        endpoint = processor.get_endpoints("tests2")
        assert endpoint.get("bot_endpoint") is None
        assert endpoint.get("action_endpoint").get("url") == "http://23.251.147.214:5000/"
        assert endpoint.get("tracker") is None
    ...
    def test_add_endpoints_add_history_endpoint(self):
        ...
        assert endpoint.get("bot_endpoint") is None
        assert endpoint.get("action_endpoint") is None
        assert endpoint.get("history_endpoint").get("url") == "http://23.251.147.214:27017/"
    ...
    def test_get_history_server_endpoint(self):
        ...
        assert endpoint.get("url") == "http://23.251.147.214:27017/"
    ...
    def test_get_kairon_history_server_endpoint(self):
        ...
        assert endpoint.get("url") == "http://23.251.147.214:8083"
    ...
    def test_update_endpoints(self):
        processor = MongoProcessor()
        config = {
            "action_endpoint": {"url": "http://23.251.147.214:8000/"},
            "bot_endpoint": {"url": "http://23.251.147.214:5000/"},
            "history_endpoint": {
                "url": "http://23.251.147.214:27019/",
                "token": "kairon-history-user",
            },
        }
        processor.add_endpoints(config, bot="tests", user="testUser")
        endpoint = processor.get_endpoints("tests")
        assert endpoint.get("bot_endpoint").get("url") == "http://23.251.147.214:5000/"
        assert endpoint.get("action_endpoint").get("url") == "http://23.251.147.214:8000/"
        assert endpoint.get("history_endpoint").get("url") == "http://23.251.147.214:27019/"
    ...
    def test_start_training_done_using_event(self, monkeypatch):
        responses.add(
            responses.POST,
            "http://23.251.147.214/train",
            status=200,
            match=[responses.json_params_matcher({"bot": "test_event", "user": "testUser", "token": None})],
        )
        monkeypatch.setitem(Utility.environment['model']['train'], "event_url", "http://23.251.147.214/train")
    ...
    def test_start_training_done_reload_event(self, monkeypatch):
        ...
        responses.add(
            responses.GET,
            f"http://23.251.147.214/api/bot/{bot}/reload",
            json='{"message": "Reloading Model!"}',
            status=200
        )
        monkeypatch.setitem(Utility.environment['model']['agent'], "url", "http://23.251.147.214/")
    ...
    def test_start_training_done_reload_event_without_token(self, monkeypatch):
        ...
        responses.add(
            responses.GET,
            f"http://23.251.147.214/api/bot/tests/reload",
            json='{"message": "Reloading Model!"}',
            status=200
        )
        model_path = start_training("tests", "testUser")
    ...
    def test_push_notifications_enabled_create_type_event(self):
        ...
        user = 'test'
        url = f"http://23.251.147.214/events/{bot}"
    ...
    def test_push_notifications_enabled_update_type_event(self):
        ...
        bot = "tests"
        user = 'testUser'
        url = "http://23.251.147.214/events"
        Utility.environment['notifications']['server_endpoint'] = url
        processor = MongoProcessor()
...
class TestModelProcessor:
    ...
    def test_save_auditlog_event_config_without_headers(self):
        bot = "tests"
        user = "testuser"
        data = {"ws_url": "http://23.251.147.214:5000/event_url"}
        MongoProcessor.save_auditlog_event_config(bot=bot, user=user, data=data)
    ...
    def test_save_auditlog_event_config(self):
        ...
        responses.add(
            responses.GET,
            "http://23.251.147.214:5000/event_url",
            status=200,
