...
from kairon.idp.constants import IDPURLConstants, IDPClientNames
...
class TestIDP:
    ...
    def set_idp_props(self, monkeypatch):
        ...
        monkeypatch.setitem(Utility.environment['idp'], 'server_url', 'http://23.251.147.214:8080/auth')
        ...
        monkeypatch.setitem(Utility.environment['idp'], 'callback_frontend_url', 'https://23.251.147.214:3000')
    ...
    def test_get_admin_access_token(self, monkeypatch, set_idp_props):
        event_url = "http://23.251.147.214:8080/auth" + IDPURLConstants.AUTH_TOKEN_URL.value.format(realm_name="master")
        responses.reset()
        response_data = {
    ...
    def test_save_keycloak_config_new(self, monkeypatch, set_idp_props):
        ...
        assert redirect_url == f"http://23.251.147.214:8080/auth/realms/{realm_name}/broker/{realm_name}/endpoint"
    ...
    def test_get_idp_config(self, set_idp_props):
        ...
        assert idp_config["idp_redirect_uri"] == f"http://23.251.147.214:8080/auth/realms/{realm_name}/broker/{realm_name}/endpoint"
    ...
    def test_get_realm_clients(self, monkeypatch, set_idp_props):
        ...
        responses.add(
            "GET", url, status=200, json=get_idp_clients_list()
        )
    ...
    def test_identify_user_and_create_access_token_new_user(self, monkeypatch, set_idp_props):
        def _get_idp_token(*args, **kwargs):
            return {"email": "test_sso_user@demo.in",
                    "given_name": "test",
                    "family_name": "user"}
