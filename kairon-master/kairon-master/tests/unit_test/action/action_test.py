...
class TestActions:
    ...
    def test_execute_http_request_get_with_auth_token(self):
        http_url = 'http://23.251.147.214:8080/mock'
    ...
    def test_prepare_url_with_path_params_and_query_params(self):
        http_url = 'http://23.251.147.214:8080/mock/$SENDER_ID/$INTENT/$USER_MESSAGE/$$KEY_VAULT/$$AWS/$email?sender_id=$SENDER_ID&userid=1011&intent=$INTENT&msg=$USER_MESSAGE&key=$$KEY_VAULT&aws=$$AWS&email=$email'
        ...
        assert updated_url == 'http://23.251.147.214:8080/mock/987654321/greet/hello/123456789-0lmgxzxdfghj/435fdr/udit.pandey@digite.com?sender_id=987654321&userid=1011&intent=greet&msg=hello&key=123456789-0lmgxzxdfghj&aws=435fdr&email=udit.pandey@digite.com'

        tracker_data['user_message'] = '/custom_action{"kairon_user_msg": "what is digite?"}'
        tracker_data[KAIRON_USER_MSG_ENTITY] = "what is digite?"
        updated_url = ActionUtility.prepare_url(http_url, tracker_data)
        assert updated_url == 'http://23.251.147.214:8080/mock/987654321/greet/what is digite?/123456789-0lmgxzxdfghj/435fdr/udit.pandey@digite.com?sender_id=987654321&userid=1011&intent=greet&msg=what is digite?&key=123456789-0lmgxzxdfghj&aws=435fdr&email=udit.pandey@digite.com'
    ...
    def test_prepare_url_with_multiple_placeholders(self):
        http_url = 'http://23.251.147.214:8080/mock?sender_id1=$SENDER_ID&intent1=$INTENT&msg1=$USER_MESSAGE&key1=$$KEY_VAULT&email1=$email&sender_id=$SENDER_ID&intent=$INTENT&msg=$USER_MESSAGE&key=$$KEY_VAULT&email=$email/'
        tracker_data = {'slot': {"email": "udit.pandey@digite.com", "firstname": "udit"},
                        'sender_id': "987654321", "intent": "greet", "user_message": "hello",
                        'key_vault': {'EMAIL': 'nkhare@digite.com', 'KEY_VAULT': '123456789-0lmgxzxdfghj'}}
        updated_url = ActionUtility.prepare_url(http_url, tracker_data)
        assert updated_url == 'http://23.251.147.214:8080/mock?sender_id1=987654321&intent1=greet&msg1=hello&key1=123456789-0lmgxzxdfghj&email1=udit.pandey@digite.com&sender_id=987654321&intent=greet&msg=hello&key=123456789-0lmgxzxdfghj&email=udit.pandey@digite.com/'

    def test_prepare_url_with_expression_failed_evaluation(self):
        http_url = 'http://23.251.147.214:8080/mock?sender_id=$SENDER_ID&intent=$INTENT&msg=$USER_MESSAGE&email=$email/'
        tracker_data = {'slot': {"email": "udit.pandey@digite.com", "firstname": "udit"}}
        updated_url = ActionUtility.prepare_url(http_url, tracker_data)
        assert updated_url == 'http://23.251.147.214:8080/mock?sender_id=&intent=&msg=&email=udit.pandey@digite.com/'

    def test_prepare_url_with_expression_failed_evaluation_with_keyvault(self):
        http_url = 'http://23.251.147.214:8080/mock?sender_id=$SENDER_ID&intent=$INTENT&msg=$USER_MESSAGE&key=$$KEY_VAULT&email=$email/'
        tracker_data = {'slot': {"email": "udit.pandey@digite.com", "firstname": "udit"}}
        updated_url = ActionUtility.prepare_url(http_url, tracker_data)
        assert updated_url == 'http://23.251.147.214:8080/mock?sender_id=&intent=&msg=&key=&email=udit.pandey@digite.com/'
    ...
    async def test_run_with_get(self, monkeypatch):
        action = HttpActionConfig(
            ...
            http_url="http://23.251.147.214:8081/mock",
            request_method="GET",
            params_list=None,
            bot="5f50fd0a56b698ca10d35d2e",
            user="user"
    ...
    async def test_run_no_connection(self, monkeypatch):
        ...
        action = HttpActionConfig(
            ...
            http_url="http://23.251.147.214:8085/mock",
            request_method="GET",
            params_list=None,
            bot="5f50fd0a56b698ca10d35d2e",
            user="user"
    ...
    async def test_run_with_get_placeholder_vs_string_response(self, monkeypatch):
        ...
        monkeypatch.setattr(ActionUtility, "get_action", _get_action)
        http_url = 'http://23.251.147.214:8082/mock'
    ...
    def test_get_email_action_config(self):
        with patch('kairon.shared.utils.SMTP', autospec=True) as mock:
            ...
            assert expected['action_name'] == actual['action_name']
            assert expected['response'] == actual['response']
            assert 'test.23.251.147.214' == actual['smtp_url']
