...
def endpoint_details(*args, **kwargs):
    return {"history_endpoint": {"url": "https://23.251.147.214:8083", "token": "test_token"}}
...
def test_chat_history_users_kairon_client_user_endpoint(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/conversations/users?from_date={from_date}&to_date={to_date}",
        status=200,
...
def test_chat_history_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/conversations/users/5e564fbcdcf0d5fad89e3acd"
        f"?from_date={from_date}&to_date={to_date}",
        status=200,
...
def test_chat_history_with_kairon_client_with_special_character(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f'https://23.251.147.214:8083/api/history/{pytest.bot}/conversations/users/LNLMC1/daIk='
        f'?from_date={from_date}&to_date={to_date}',
        status=200,
        json={"data": {"history": history_conversations()[0]}},
        match=[responses.json_params_matcher({})],
...
def test_fallback_count_range_no_nlu_fallback_rule(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/fallback"
...
def test_visitor_hit_fallback_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/fallback?from_date={from_date}&to_date={to_date}"
...
def test_conversation_steps_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/conversation/steps?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
        json={"data": 100}
    )
...
def test_conversation_time_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/conversation/time?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
        json={"data": 900.5}
    )
...
def test_user_with_metrics_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/users?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_engaged_users_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/users/engaged"
...
def test_new_users_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/users/new?from_date={from_date}&to_date={to_date}",
        match=[responses.json_params_matcher({})],
        status=200,
...
def test_successful_conversation_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/conversation/success?from_date={from_date}"
...
def test_user_retention_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/users/retention?from_date={from_date}&to_date={to_date}",
        match=[responses.json_params_matcher({})],
        status=200,
...
def test_engaged_user_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/users/engaged?from_date={from_date}&to_date={to_date}"
...
def test_new_user_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/users/new?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_successful_conversation_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/conversations/success?from_date={from_date}"
        f"&to_date={to_date}&action_fallback=action_default_fallback&nlu_fallback=utter_please_rephrase",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_user_retention_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/users/retention?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_fallback_count_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/fallback?from_date={from_date}&to_date={to_date}"
        f"&action_fallback=action_default_fallback&nlu_fallback=utter_please_rephrase",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_flat_conversations_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/conversations/?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_download_conversation_with_data_with_kairon_client(mock_auth_admin, mock_mongo_processor, mock_list_bots):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/conversations/download?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
        body=file.read(),
...
def test_total_conversation_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/conversations/total?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_top_intent_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/intents/topmost?from_date={from_date}"
...
def test_top_action_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/actions/topmost?from_date={from_date}"
        f"&to_date={to_date}&top_n=10",
        match=[responses.json_params_matcher({})],
        status=200,
...
def test_conversation_step_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/conversations/steps?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_wordcloud_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/conversations/wordcloud?from_date={from_date}"
...
def test_unique_user_input_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/users/input?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
        json={"data": [{'_id': 'nlu_fallback', 'count': 32}]}
    )
...
def test_conversation_time_range_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/trends/conversations/time?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
        json={"data": {'total_conversation_range': {1: 25, 2: 24, 3: 28, 4: 26, 5: 20, 6: 25}}}
    )
...
def test_dropoff_users_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/fallback/dropoff?from_date={from_date}"
        f"&to_date={to_date}&action_fallback=action_default_fallback&nlu_fallback=utter_please_rephrase",
        status=200,
        match=[responses.json_params_matcher({})],
        json={"data": {'total_conversation_range': {1: 25, 2: 24, 3: 28, 4: 26, 5: 20, 6: 25}}}
...
def test_user_intent_dropoff_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/intents/dropoff?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
        json={"data": {'total_conversation_range': {1: 25, 2: 24, 3: 28, 4: 26, 5: 20, 6: 25}}}
    )
...
def test_unsuccessful_session_count_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/sessions/unsuccessful?from_date={from_date}"
        f"&to_date={to_date}&action_fallback=action_default_fallback&nlu_fallback=utter_please_rephrase",
        status=200,
        match=[responses.json_params_matcher({})],
...
def test_total_sessions_with_kairon_client(mock_auth, mock_mongo_processor):
    ...
    responses.add(
        responses.GET,
        f"https://23.251.147.214:8083/api/history/{pytest.bot}/metrics/sessions/total?from_date={from_date}&to_date={to_date}",
        status=200,
        match=[responses.json_params_matcher({})],
