...
def test_augment_questions():
    responses.add(
        responses.POST,
        url="http://23.251.147.214:8000/questions",
...
def endpoint_response(*args, **kwargs):
    return {"bot_endpoint": {"url": "http://23.251.147.214:5000"}}
...
def test_deploy(mock_endpoint):
    responses.add(
        responses.PUT,
        "http://23.251.147.214:5000/model",
        status=204,
    )
    response = client.post(
        f"/api/bot/{pytest.bot}/deploy",
...
def test_augment_paraphrase_gpt():
    responses.add(
        responses.POST,
        url="http://23.251.147.214:8000/paraphrases/gpt",
...
def test_augment_paraphrase():
    responses.add(
        responses.POST,
        "http://23.251.147.214:8000/paraphrases",
        json={
            "success": True,
            "data": {
...
def test_save_endpoint(monkeypatch):
    ...
    responses.add(
        responses.GET,
        f"http://23.251.147.214/api/bot/{pytest.bot}/reload",
        status=200,
    ...
    response = client.put(
        f"/api/bot/{pytest.bot}/endpoint",
        headers={"Authorization": pytest.token_type + " " + pytest.access_token},
        json={"bot_endpoint": {"url": "http://23.251.147.214:5005/"},
              "action_endpoint": {"url": "http://23.251.147.214:5000/"},
              "history_endpoint": {"url": "http://23.251.147.214", "token": "rasa234568"}}
    )
...
def test_get_history_endpoint():
    ...
    actual = response.json()
    assert actual['data']['endpoint']['history_endpoint']['url'] == "http://23.251.147.214"
...
def test_chat(monkeypatch):
    ...
    responses.add(
        responses.POST,
        f"http://23.251.147.214/api/bot/{pytest.bot}/chat",
        status=200,
        match=[
            responses.json_params_matcher(
                chat_json)],
...
def test_add_channel_config(monkeypatch):
    monkeypatch.setitem(Utility.environment['model']['agent'], 'url', "http://23.251.147.214:5056")
    data = {"connector_type": "slack",
            "config": {
                "bot_user_oAuth_token": "xoxb-801939352912-801478018484-v3zq6MYNu62oSs8vammWOY8K",
                "slack_signing_secret": "79f036b9894eef17c064213b90d1042b",
    ...
    assert actual["data"].startswith(f"http://23.251.147.214:5056/api/bot/slack/{pytest.bot}/e")
